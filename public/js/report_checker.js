...
function canvas_to_file(cs_id, su_round) {
    canvas_1.toBlob((blob) => {
        ...
        sign_checker(cs_id, su_round, file);
    }, 'image/png');
}

//담당자 서명 API
function sign_checker(cs_id, su_round, file) {
    var tmpForm = new FormData();
    tmpForm.append('r8_checker_sign_image', file);

    $.ajax({
        url: `http://api.insafed.com:3000/v1/cons/${cs_id}/report/8/sign/checker?round=${su_round}`,
        type: 'POST',
